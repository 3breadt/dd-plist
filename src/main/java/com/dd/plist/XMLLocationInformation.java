 ...
 *
 * <p>Line and column number are only available if the {@code withLineInformation} parameter in the
 * call to one of the {@link XMLPropertyListParser}'s parse methods was set to {@code true}. The
 * line information is taken from the {@link Locator} class and thus is only an approximation of the
 * actual location that can only be used for diagnostic purposes.
 *
...
public class XMLLocationInformation extends LocationInformation {
  ...
  XMLLocationInformation(Node n, String xpath) {
    ...
    if (n.hasAttributes()) {
      ...
      Node lineNumberNode =
          attrs.getNamedItemNS(XMLLocationFilter.NS, XMLLocationFilter.LINE_NUMBER);
      ...
      Node colNumberNode =
          attrs.getNamedItemNS(XMLLocationFilter.NS, XMLLocationFilter.COLUMN_NUMBER);
