...
public class BinaryPropertyListParserTest {
  ...
  private static final byte[] HUGE_POSITIVE_4 =
      new byte[] {0x7F, (byte) 0xFF, (byte) 0xFF, (byte) 0xF0};
  private static final byte[] NEGATIVE_4 = new byte[] {(byte) 0x80, 0x00, 0x00, 0x00};
  private static final byte[] NEGATIVE_8 =
      new byte[] {
  ...
  public void parse_providesCorrectObjectLocations() throws Exception {
    ...
    // each dictionary key is serialized as an NSObject, as we have 5 keys, the next value object
    // has ID 6
    ...
    assertThat(
        d.get("key&B").getLocationInformation(), instanceOf(BinaryLocationInformation.class));
    assertThat(
        ((BinaryLocationInformation) d.get("key&B").getLocationInformation()).getId(), is(7));
    ...
    assertThat(
        array.objectAtIndex(0).getLocationInformation(),
        instanceOf(BinaryLocationInformation.class));
    assertThat(
        ((BinaryLocationInformation) array.objectAtIndex(0).getLocationInformation()).getId(),
        is(11));
    assertThat(
        array.objectAtIndex(1).getLocationInformation(),
        instanceOf(BinaryLocationInformation.class));
    assertThat(
        ((BinaryLocationInformation) array.objectAtIndex(1).getLocationInformation()).getId(),
        is(12));
    assertThat(
        array.objectAtIndex(2).getLocationInformation(),
        instanceOf(BinaryLocationInformation.class));
    assertThat(
        ((BinaryLocationInformation) array.objectAtIndex(2).getLocationInformation()).getId(),
        is(13));
    assertThat(
        array.objectAtIndex(3).getLocationInformation(),
        instanceOf(BinaryLocationInformation.class));
    assertThat(
        ((BinaryLocationInformation) array.objectAtIndex(3).getLocationInformation()).getId(),
        is(14));
  }
  ...
  public void parse_canHandleNumbersWithInfinityValue() throws Exception {
    NSDictionary dict =
        (NSDictionary) BinaryPropertyListParser.parse(new File("test-files/infinity-binary.plist"));
  ...
  public void parse_rejectsExcessivelyNestedStructures() {
    // https://github.com/3breadt/dd-plist/issues/104 (additional finding: unbounded recursion
    // depth)
  ...
  private static byte[] buildSingleObjectPlist(
      int typeNibble, byte[] lengthMarker, byte[] lengthValue) throws IOException {
  ...
  private static byte[] buildDeeplyNestedArrayPlist(int depth) {
    ...
    writeInt32BE(
        data, tablePos, headerLength + depth * arrayLength); // offset of the terminal object
